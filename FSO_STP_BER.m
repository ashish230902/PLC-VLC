...
noise_x =(0:0.01:80);
le = [500,1000,2000,3000,4000,5000]; %link length
Rdia= 5*10^-3; %aperture of Rx
wav= 1550*10^-9;
h_upperlimit = 6; %Upperlimit of h
...
%% Gamma-Gamma
for k=1:length(le)
sigma_sq = 1.23*Cn_sq*(k_beam^(7/6))*le(k)^(11/6);

dsig    = sqrt((0.25*k_beam*(Rdia)^2)/le(k));
...
%% SNR
 ...
 N0= (Pt_av.*h2./SNR_th);
   ...
   SNR_dB=[0,5,10,15,20,25,30,35,40]; %SNR value in dB
   SNR=10.^(SNR_dB./10); 
  
  %SNR= SNR.*h2;
  for j=1:length(SNR_dB)
  Noise_var_pdf=sqrt(Pt_av./(SNR(j)));
 ...
 BER_plot(k,j)= BER;
  ...
  for i=1:length(le)
      figure(4)
          plot(SNR_dB,STP_plot(i,:))
          hold on       
  end
  
  hold off
  legend('500','1000','2000','3000','4000','5000')
  
  for i=1:length(le)
      figure(5)
          semilogy(SNR_dB,BER_plot(i,:))
          hold on       
  end
  hold off
  legend('500','1000','2000','3000','4000','5000')
